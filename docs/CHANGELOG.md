...
- **유닛 몸체 사선 회전 수정** (MachineEntity.kt, EnemyTankEntity.kt)
  ...
  - 적 탱크: 기본 -90° (하방) 고정, 무기만 타겟 방향 회전 (기존 동작 유지)
  - 죽은 코드 정리: EMA 스무딩, 데드존 히스테리시스, 회전 상수/필드 74줄 제거
- **HP 게이지 겹침 해소** (StageScreen.kt)
  - ScrollPane 상단이 HP 게이지(Y=989) 뒤까지 확장되어 기지 건물을 가리던 문제
  - ScrollPane 높이를 35px (게이지 32px + 여백 3px) 줄여 겹침 제거
